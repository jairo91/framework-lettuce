...
let Keywords = class Keywords {
	...
	*/
	...
	/**
				* Este método elimina todos los elementos guardados en la ejecucción
	*/
	deleteData() {
		logger.debug('Deleting saved data...');
		data = {};
	}
	...
	pushOn(element, timeOut = 10) {
		...
		try {
			if (isInternalId) {
				...
				$(element).waitForDisplayed({ timeout: timeOut * 1000 });
				$(element).click();
			} else {
		...
		this._AttachmentCommand('pushOn');
	}
	...
	*/
	writeInto(element, value, errorMessage = 'Element is not found or it is not possible insert --> ' + element, timeOut = 10) {
		...
		try {
			...
			this._AttachmentCommand('writeInto');
		}
	}
		...
		if (element !== null && element !== undefined && element !== '') {
			if (text !== null && text !== undefined) {
				...
				if (!this.equalValues(textElement, text) && verifyEquals === true) {
					...
					this._AttachmentCommand('verifyText');
	...
	*/
	selectOption(element, option) {
		...
		this._AttachmentCommand('selectOption');
	}
		...
		if (element !== null && element !== undefined && element !== '') {
			if (text !== null && text !== undefined && text !== '') {
				...
				if (!textElement.includes(text)) {
					this._AttachmentCommand('containsText');
					throw Error('The given text: \'' + text + '\' is not equals in the element : \'' + textElement + '\'');
				} else {
					logger.debug('Text \'' + text + '\' found');
					return true;
	...
	*/
	waitToBePresent(element, timeOut = 10) {
		...
		try {
			...
			this._AttachmentCommand('waitToBePresent');
	...
	*/
	waitToVanish(element, timeOut = 10) {
		...
		if (flag) {
			this._AttachmentCommand('waitToVanish');
	...
	*/
	exists(element, timeOut = 10, errorMessage = 'Verifying that \'' + element + '\' is existing in ' + timeOut + 's.') {
		logger.debug(errorMessage);
		try {
			$(element).waitForExist({ timeout: timeOut * 1000 });
			return true;
		} catch (error) {
			return false;
		}
	...
	*/
	waitForExist(element, timeOut = 10) {
		...
		try {
			...
			this._AttachmentCommand('waitForExist');
			throw Error('The given element: \'' + element + '\' is not present in ' + timeOut + 's.');
		}
	}
	...
	*/
	executeScript(script, arg) {
		...
		this._AttachmentCommand('executeScript');
	}
	...
	*/
	scrollIntoView(element, top = false, timeOut = 10) {
		...
		if ($(element).waitForExist({ timeout: timeOut * 1000 }))
	...
	*/
	...
	/**
	 * Este método realiza la subida de un fichero sobre un campo input.
	 *
	 * @param timeOut Contiene el tiempo(segundos) de espera al elemento. Por defecto contiene 10 segundos
	 * @param selector Contiene el selector del elemento input sobre el que se incluir el fichero
	 * @param path Path de la ruta del fichero en la maquina local. Por ejemplo: "../../data/contratoEjemplo.pdf"
	*/
	uploadFile(selector, relativePath, timeOut = 10) {
		logger.debug('Upload file: ' + relativePath + ' in ' + selector + ' in ' + timeOut + 's.');
		$(selector).waitForExist({ timeout: timeOut * 1000 });
		const remoteFilePath = browser.uploadFile(relativePath);
		$(selector).setValue(remoteFilePath);
	}


	/**
	 * Este método realiza la acción de verificar que el elemento no es clickable
	 *
	 * @param selector Contiene el selector del elemento que queremos verificar que es clickable
	 * @param timeOut Contiene el tiempo(segundos) de espera al elemento. Por defecto contiene 10 segundos
	*/

	isNotClickable(selector, timeOut = 10) {
		logger.debug('Verifying that \'' + selector + '\' is not clickable ' + timeOut + 's.');
		$(selector).waitForDisplayed({ timeout: timeOut * 1000 });
		let clickable = $(selector).isClickable();
		console.log(clickable);
		if (clickable === false)
			logger.debug('The given element: \'' + selector + '\' is not clickable ' + timeOut + 's.');
		else
			throw Error('The given element: \'' + selector + '\' is clickable ' + timeOut + 's.');
	}

	/**
	 * Este método realiza la acción de verificar que el elemento es clickable
	 *
	 * @param selector Contiene el selector del elemento que queremos verificar que es clickable
	 * @param timeOut Contiene el tiempo(segundos) de espera al elemento. Por defecto contiene 10 segundos
	*/

	isClickable(selector, timeOut = 10) {
		logger.debug('Verifying that \'' + selector + '\' is clickable ' + timeOut + 's.');
		$(selector).waitForDisplayed({ timeout: timeOut * 1000 });
		let clickable = $(selector).isClickable();
		console.log(clickable);
		if (clickable)
			logger.debug('The given element: \'' + selector + '\' is clickable ' + timeOut + 's.');
		else
			throw Error('The given element: \'' + selector + '\' is not clickable ' + timeOut + 's.');
	}

	/**
	 * Este método realiza la acción de verificar que el elemento no esta habilitado
	 *
	 * @param selector Contiene el selector del elemento que queremos verificar que no esta habilitado
	 * @param timeOut Contiene el tiempo(segundos) de espera al elemento. Por defecto contiene 10 segundos
	*/
	isNotEnabled(selector, timeOut = 10) {
		logger.debug('Verifying that \'' + selector + '\' is not enabled ' + timeOut + 's.');
		$(selector).waitForDisplayed({ timeout: timeOut * 1000 });
		let enabled = $(selector).isEnabled();
		console.log(enabled);
		if (enabled === false)
			logger.debug('The given element: \'' + selector + '\' is not enabled ' + timeOut + 's.');
		else
			throw Error('The given element: \'' + selector + '\' is enabled ' + timeOut + 's.');
	}

	/**
	 * Este método realiza la acción de verificar que el elemento esta habilitado
	 *
	 * @param selector Contiene el selector del elemento que queremos verificar que esta habilitado
	 * @param timeOut Contiene el tiempo(segundos) de espera al elemento. Por defecto contiene 10 segundos
	*/
	isEnabled(selector, timeOut = 10) {
		logger.debug('Verifying that \'' + selector + '\' is enabled ' + timeOut + 's.');
		$(selector).waitForDisplayed({ timeout: timeOut * 1000 });
		let enabled = $(selector).isEnabled();
		console.log(enabled);
		if (enabled)
			logger.debug('The given element: \'' + selector + '\' is enabled ' + timeOut + 's.');
		else
			throw Error('The given element: \'' + selector + '\' is not enabled ' + timeOut + 's.');
	}
};
