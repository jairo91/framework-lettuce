	...
	*/
	...
	browser.addCommand('pushOn', function (element, timeOut = 10) {
		...
		try {
			...
			logger.debug("Pushing via javascript on ", element);
			try {
				if (element.indexOf("//") !== -1) {
					browser.execute("document.evaluate(\"" + element + "\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click();");
				} else {
					browser.execute("document.querySelector(\"" + element + "\").click();");
				}
			} catch (error) {
