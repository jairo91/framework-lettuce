...
/**
 * Este método devuelve el nivel de log incluido en el fichero configuración de webdriverio
 *
 * @returns el nivel de log seleccionado en el fichero de configuración
*/
function getLogLevel() {
	try {
		return browser.loggerLevel;
	} catch (error) {
		return 'debug';
	}
}
...
let self = {
	...
	error: function (message, obj = "") {
		if (_logger.levels[getLogLevel()] >= _logger.levels['error']) console.log(getDate() + " Error --> " + message, obj);
	},
	warn: function (message, obj = "") {
		if (_logger.levels[getLogLevel()] >= _logger.levels['warn']) console.log(getDate() + " Warn --> " + message, obj);
	},
	debug: function (message, obj = "") {
		if (console.log(getDate() + " Debug --> " + message, obj));
	},
	info: function (message, obj = "") {
		if (_logger.levels[getLogLevel()] >= _logger.levels['info']) console.log(getDate() + " Info --> " + message, obj);
	},
	silly: function (message, obj = "") {
		if (_logger.levels[getLogLevel()] >= _logger.levels['silly']) console.log(getDate() + " Silly --> " + message, obj);
	}
};
module.exports = self;
